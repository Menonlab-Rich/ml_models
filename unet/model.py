...
class UNet(nn.Module):
    ...
    UNet architecture
    '''
    def __init__(self, in_channels=3, out_channels=3, features=[64, 128, 256, 512]) -> None:
        super().__init__()
        # Downward path
    ...
    def forward(self, x):
        if x.shape[2] > 256:
            warnings.warn("x is larger than 256x256, this may cause memory issues")
        skip_connections = []
        ...
        for i, (up_layer, skip_connection) in enumerate(zip(self.up[::2], skip_connections)):
            x = up_layer(x)
