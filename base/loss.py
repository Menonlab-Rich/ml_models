...
class JaccardLoss(nn.Module):
    def __init__(self, num_classes=2, weights=None, smoothing=1e-6):
        ...
        self.weights = torch.tensor(weights if weights is not None else [1.0]*num_classes, dtype=torch.float32)
        # Register weights as a parameter if they need to be learnable, otherwise as a buffer
        if weights is not None and all(isinstance(w, torch.Tensor) for w in weights):
